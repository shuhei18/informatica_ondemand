...
<body class="site drawer drawer--left">
    ...
    <div class="site-main">
        ...
        <div class="profileform-container">
            ...
            <form id="profileForm"
                action="confirm_new.php?utm_source=<?php echo urlencode($utm_source); ?>&utm_medium=<?php echo urlencode($utm_medium); ?>&utm_campaign=<?php echo urlencode($utm_campaign); ?>"
                method="post">
                <button type="button" style="margin-bottom: 15px;"
                    onclick="location.href='privacy_new.php?utm_source=<?php echo urlencode($utm_source); ?>&utm_medium=<?php echo urlencode($utm_medium); ?>&utm_campaign=<?php echo urlencode($utm_campaign); ?>'"
                    class="select-btn-2">同意画面に戻る</button>
        ...
        <script>
        document.addEventListener('DOMContentLoaded', function() {
            let emailValidationError = false; // メールアドレスの形式エラー
            let emailExistsError = false; // メールアドレスの重複エラー
            let emailRestrictedError = false; // メールアドレスの競合企業エラー
            let passwordMismatchError = false; // パスワードの不一致エラー
            let passwordFormatError = false; // パスワードの形式エラー
            let corporateTypeError = false; // 法人格の選択エラー
            let corporatePrefixError = false; // 法人格の「前」「後」「なし」の選択エラー
            let katakanaError = false; // カタカナの入力エラー
            let addressValidationError = false; // 住所バリデーションエラー
            let emailMatchError = false; // メールアドレスの一致エラー
            ...
            async function validateEmail() {
                ...
                // メールアドレスが空であればエラーをクリア
                if (email === "") {
                    emailError.textContent = "";
                    emailValidationError = false;
                    emailExistsError = false;
                    emailRestrictedError = false;
                    return;
                }
                ...
                const restrictedDomains = [
                    ...
                    "yandex.ru", "aol.jp", "mail.com", "docomo.ne.jp",
                    "ezweb.ne.jp", "au.com", "softbank.ne.jp", "softbank.jp", "vodafone.ne.jp",
                    "pdx.ne.jp", "disney.ne.jp", "y-mobile.ne.jp", "willcom.com","gmail.com","icloud.com",
                    "saison-technology.com", "alation.com"
                ];
                const emailDomain = email.split('@').pop();

                // 競合企業ドメインチェック
                if (restrictedEmails.includes(email) || restrictedDomains.includes(emailDomain)) {
                    emailRestrictedError = true;
                    emailValidationError = false;
                    emailExistsError = false;
                } else {
                    emailRestrictedError = false;
            ...
            function validatePassword() {
                ...
                if (!passwordPattern.test(password)) {
                    ...
                    passwordError.textContent = "パスワードは半角英数字8~12文字で入力してください。";
            ...
            // 法人格バリデーション
            function validateCorporateType() {
                const corporateTypeElement = document.getElementById('corporate_type');
                const corporateTypeErrorElement = document.getElementById('error_corporate_type');

                if (!corporateTypeElement.value) {
                    corporateTypeErrorElement.textContent = "法人格を選択してください。";
                    corporateTypeErrorElement.scrollIntoView({
                        behavior: 'smooth',
                        block: 'center'
                    });
                    corporateTypeError = true;
                } else {
                    corporateTypeErrorElement.textContent = "";
            ...
            function validateCorporatePrefix() {
                ...
                if (!isSelected) {
                    const corporatePrefixErrorElement = document.getElementById('error_corporate_prefix');
                    corporatePrefixErrorElement.textContent = "「法人格」の「前」「後」「なし」が選択されていません。";
                    corporatePrefixErrorElement.scrollIntoView({
                        behavior: 'smooth',
                        block: 'center'
                    });
                    corporatePrefixError = true;
                } else {
                    document.getElementById('error_corporate_prefix').textContent = "";
                    corporatePrefixError = false;
                }
            }
            ...
            async function searchAddress() {
                ...
                if (postalCode.length === 7) {
                    try {
                        const response = await fetch('zipcode_database.json');
                        const data = await response.json();
                        const result = data[postalCode];

                        if (result) {
                            document.getElementById('prefecture').value = result.都道府県;
                            document.getElementById('city').value = result.市区町村;
                            document.getElementById('town').value = result.町域;
                        ...
                        postalCodeError.textContent = "住所検索中にエラーが発生しました。";
                        postalCodeError.scrollIntoView({
                            behavior: 'smooth',
                            block: 'center'
                        });
            ...
            // メール、パスワード、カタカナのリアルタイムバリデーション
            document.getElementById('work_email').addEventListener('input', async function() {
                await validateEmail();
                validateEmailMatch();
            });

            document.getElementById('work_email_confirm').addEventListener('input', function() {
                validateEmailMatch();
            });
            ...
            document.getElementById('profileForm').addEventListener('submit', async function(event) {
                ...
                validateEmailMatch();
                validatePasswordMatch();
                validateCorporateType();
                validateCorporatePrefix();

                let hasError = false;

                if (emailValidationError || emailExistsError || emailRestrictedError ||
                    emailMatchError) {
                    hasError = true;
                }

                if (passwordMismatchError || passwordFormatError) {
                    hasError = true;
                }

                if (corporateTypeError || corporatePrefixError || katakanaError) {
                    hasError = true;
                }
