...
// セッションの有効期限を30分に設定
$timeout_duration = 1800; // 30分（秒）

// 最後のアクティビティ時間を確認
if (isset($_SESSION['LAST_ACTIVITY']) && (time() - $_SESSION['LAST_ACTIVITY']) > $timeout_duration) {
    // 30分間アクティビティがない場合、セッションを破棄してログインページにリダイレクト
    session_unset(); // セッション変数を全て解除
    session_destroy(); // セッションを破棄
    header("Location: login.php");
    exit();
}
$_SESSION['LAST_ACTIVITY'] = time(); // 現在のタイムスタンプで更新
...
<div class="info-item">ログインID: <span id="loginID"><?php echo htmlspecialchars($loginID, ENT_QUOTES, 'UTF-8'); ?></span></div>
        ...
        <!-- <button onclick="generateQRCode()">登録証発行</button> -->
        <!--<div id="qrcode"></div>
         <button onclick="downloadPDF()">登録証発行（PDFをダウンロード）</button>
        <div class="print-instruction">登録証（受講票）をA4サイズ・100%で印刷し四つ折りにして会場へお持ちください</div>-->
    </div>
    <script>
