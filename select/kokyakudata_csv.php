...
SELECT 
    ...
    COALESCE(CASE WHEN a.main IS NOT NULL THEN 1 ELSE 0 END, 0) AS `出席 メイン`,
    COALESCE(CASE WHEN a.mini_a_1 IS NOT NULL THEN 1 ELSE 0 END, 0) AS `出席 NTTデータ`,
    COALESCE(CASE WHEN a.mini_b_1 IS NOT NULL THEN 1 ELSE 0 END, 0) AS `出席 SCSK`,
    COALESCE(CASE WHEN a.mini_a_2 IS NOT NULL THEN 1 ELSE 0 END, 0) AS `出席 NTT Com`,
    COALESCE(CASE WHEN a.mini_b_2 IS NOT NULL THEN 1 ELSE 0 END, 0) AS `出席 CTC`,
    COALESCE(CASE WHEN a.mini_a_3 IS NOT NULL THEN 1 ELSE 0 END, 0) AS `出席 ALSI_2`,
    COALESCE(v.video_1_flag, 0) AS `video_1_flag`,
    COALESCE(v.video_2_flag, 0) AS `video_2_flag`,
    COALESCE(v.video_3_flag, 0) AS `video_3_flag`,
    COALESCE(v.video_4_flag, 0) AS `video_4_flag`,
    COALESCE(v.video_5_flag, 0) AS `video_5_flag`,
    COALESCE(v.video_6_flag, 0) AS `video_6_flag`,
    COALESCE(v.video_7_flag, 0) AS `video_7_flag`,
    COALESCE(v.video_8_flag, 0) AS `video_8_flag`
FROM 
    users u
LEFT JOIN 
    user_lectures ul ON u.loginID = ul.loginID
LEFT JOIN 
    utm_campaign_source t ON (
        CASE
...
LEFT JOIN 
    attendance a ON u.loginID = a.loginID
LEFT JOIN 
    user_video_flags v ON u.loginID = v.loginID  -- user_video_flags テーブルを追加
GROUP BY 
    ...
    v.video_1_flag, v.video_2_flag, v.video_3_flag, v.video_4_flag, v.video_5_flag,
    v.video_6_flag, v.video_7_flag, v.video_8_flag
ORDER BY u.id
